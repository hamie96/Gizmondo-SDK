...
psp_fmgr_get_dir_list(char *basedir, int dirmax, char **dirname)
{
  ...
  for (index = 0; index < nfiles; index++) {
    dirname[index] = Util_strdup(sortfiles[index]->d_name);
  }
  return nfiles;
}
...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back2_print(170,  80, "press Stop to confirm !", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  ...
  do
  {
    ...
    if (c.Buttons & GP2X_CTRL_STOP) { confirm = 1; break; }
...
static  int sel=0;
  ...
  for(;;) 
  {
    ...
    if ((new_pad & GP2X_CTRL_PLAY) || (new_pad & GP2X_CTRL_FFWD)) {
    ...
    } else if(new_pad & GP2X_CTRL_RWND){
      up=1;
    } else if((new_pad & GP2X_CTRL_STOP)) {
      /* Cancel */
      file_selected = 0;
      break;
      ...
      sel+=10;
    } else if(new_pad & GP2X_CTRL_POWER){
      if (sortfiles[sel]->d_type != DT_DIR) {
        display_screen = 1;
        strcpy(out, path);
        strcat(out, sortfiles[sel]->d_name);
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
	...
	if (strlen(user_filedir_set))
		strcat(user_filedir_set, "\\set\\");
	else
		strcat(user_filedir_set, "set\\");

	if (strlen(user_filedir_rom))
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    error = 0;
    if (!Util_fileexists(user_filename)) error = 1;
    else 
    {
