...
int OpenSoundFile(const char *szFileName)
{
	...
	*/
	...
	if (fwrite("\0\x10\0data\0\0\0\0", 1, 7, sndoutput) != 7) {
		fclose(sndoutput);
		sndoutput = NULL;
		return FALSE;
	}
...
int WriteToSoundFile(const unsigned char *buffer, unsigned int uiSize)
{
	unsigned short * b = (unsigned short *)buffer;
	if (sndoutput && b && uiSize) {
		int i=0;
		for (i=0; i<uiSize; ++i)
		{
			int value = b[i];
			value-=32768;
			fputc((value&0xff),sndoutput);
			fputc((value&0xff00)>>8,sndoutput);
		}
		byteswritten += uiSize*2;
		return uiSize*2;
	}
