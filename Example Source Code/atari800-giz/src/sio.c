...
Util_tmpbufdef(static, sio_tmpbuf[MAX_DRIVES*2])
...
int SIO_Mount(int diskno, const char *filename, int b_open_readonly)
{
	...
	// ZIP first (so we can chain zip/dcm I hope!)
	isZip = FALSE;
	if (inZip(filename))
	{
		{
			FILE *f2 = Util_tmpopen(sio_tmpbuf[MAX_DRIVES+diskno - 1]);
			if (f2 == NULL)
				return FALSE;
			if (!CompressedFile_ExtractFromZIP(filename, f2)) {
				Util_fclose(f2, sio_tmpbuf[MAX_DRIVES+diskno - 1]);
				return FALSE;
			}
			f = f2;
			isZip = TRUE;
	...
	case 0xfa:
		/* DCM */
		{
			...
			{
				if (isZip)
					Util_fclose(f,sio_tmpbuf[MAX_DRIVES+diskno-1]);
				return FALSE;
			}
			Util_rewind(f);
			success = CompressedFile_DCMtoATR(f, f2);
	...
	case 0x1f:
		if (header.magic2 == 0x8b) {
			...
			{
				if (isZip)
					Util_fclose(filein,sio_tmpbuf[MAX_DRIVES+diskno-1]);
				return FALSE;
			}
			Util_rewind(filein);
			success =  CompressedFile_ExtractGZF(filein, f);
			if (isZip)
			{
				Util_fclose(filein, sio_tmpbuf[MAX_DRIVES+diskno-1]);
				isZip = FALSE;
	...
	if (isZip)
	{
		// we used a second temporary buffer for the zip tempfile, so the zip may
		// contain other compressed formats
		// copy it to the usual place
#ifndef HAVE_TMPFILE
#ifdef HAVE_UTIL_UNLINK
		strcpy (sio_tmpbuf[diskno-1], sio_tmpbuf[MAX_DRIVES+diskno-1]);
		strcpy (sio_tmpbuf[MAX_DRIVES+diskno-1], "");
#endif
#endif
	}
