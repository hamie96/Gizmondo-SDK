...
#define MENU_CAPTURE 27
#define MENU_STARTUPLIKETHIS 28
#define MENU_DELETE 29
...
extern int gp32ClockEnum; //positions must obviously match enum!
...
static void DiskManagement(void)
{
	...
	static tMenuItem menu_array[] = {
		...
		MENU_FILESEL_TIP(12, "Uncompress Disk Image", "Convert GZ/DCM/ZIP to ATR"),
		MENU_END
	};
	...
	for (;;) {
		...
		switch (dsknum) {
		case 8:
			if (ui_driver->fGetSaveFilename("dst", set_filename, saved_files_dir, n_saved_files_dir)) {
...
//					fprintf(fp, "%s\n", sio_filename[i]);
		...
		case 10:
			...
			break;
		case 11:
			if (ui_driver->fGetSaveFilename("atr", disk_filename, atari_files_dir, n_atari_files_dir)) {
		...
		case 12:
			if (ui_driver->fGetLoadFilename(disk_filename, atari_files_dir, n_atari_files_dir)) {
				...
				case 0x1f:
					fclose(fp);
					if (ui_driver->fGetSaveFilename("atr",uncompr_filename, atari_files_dir, n_atari_files_dir)) {
	...
	for (;;) {
		...
		case 0:
			if (ui_driver->fGetLoadFilename(cart_filename, atari_files_dir, n_atari_files_dir)) {
				...
				if (!ui_driver->fGetSaveFilename("rom",cart_filename, atari_files_dir, n_atari_files_dir))
					break;
...
static void SoundRecording(void)
{
	if (!IsSoundFileOpen()) {
		...
		if (ui_driver->fGetSaveFilename("wav",buffer, saved_files_dir, n_saved_files_dir)) 
		{
...
{
	static tMenuItem clock_menu_array[] =
	{
		MENU_ACTION_TIP(GPMHZ_67, "Crawl", "67MHz"),
		MENU_ACTION_TIP(GPMHZ_80,"Slow", "80MHz"),
		MENU_ACTION_TIP(GPMHZ_99,"Chugging", "99MHz"),
		MENU_ACTION_TIP(GPMHZ_115,"Usable", "115MHz"),
		MENU_ACTION_TIP(GPMHZ_132,"Normal", "132MHz"),
		MENU_ACTION_TIP(GPMHZ_156,"Turbo", "156MHz"),
		MENU_ACTION_TIP(GPMHZ_165,"Drink tose batteries!", "165MHz"),
		MENU_ACTION_TIP(GPMHZ_180,"Smoking!", "180MHz"),
#ifdef GP2X
		MENU_ACTION_TIP(GPMHZ_200,"On fire", "200MHz"),
		MENU_ACTION_TIP(GPMHZ_250,"Burnt out", "250MHz"),
#endif
		MENU_END
	};
	
	static tMenuItem menu_array[] = {
		MENU_CHECK(0, "Disable BASIC (hold Option):"),
		MENU_CHECK(1, "Boot from tape (hold Start):"),
		//MENU_SUBMENU_SUFFIX(21, "Clock speed (saved in state):", NULL),
		//(no clock!)MENU_CHECK(2, "Enable R-Time 8:"),
		MENU_CHECK(3, "SIO patch (fast disk access):"),
		MENU_ACTION(4, "H: device (sd card):"),
	...
	//	MENU_FILESEL_PREFIX(10, "H4: ", atari_h_dir[3]),
		...
		MENU_FILESEL_PREFIX(11, " UI INPUT: ", atari_uimenu_filename),
		MENU_FILESEL_PREFIX(22, "    INPUT: ", atari_menu_filename),
//		MENU_FILESEL_PREFIX(23, " CLOCKCMD: ", atari_clockcommand_filename),
		...
		MENU_FILESEL(17, "Find ROM/cfg in a directory"),
	...
	for (;;) {
		...
		//FindMenuItem(menu_array,21)->suffix = FindMenuItem(clock_menu_array,gp32ClockEnum)->item;
		...
		case 21:
			option2 = ui_driver->fSelect(NULL, SELECT_POPUP, gp32ClockEnum, clock_menu_array, NULL);
			if (option2 >= 0) {
				GP32_SetClock(option2);
			}
			break;
		default:
...
{
	if (ui_driver->fGetSaveFilename("a8",state_filename, saved_files_dir, n_saved_files_dir)) {
...
static void DisplaySettings(void)
{
	...
	static const tMenuItem brightness_menu_array[] = 
	{
		MENU_ACTION(0,"Black"),
		MENU_ACTION(1,"Dark"),
		MENU_ACTION(2,"Normal"),
		MENU_ACTION(3,"Bright"),
		MENU_ACTION(4,"White"),
		MENU_END
	};
...
#endif
		MENU_SUBMENU_SUFFIX(7, "Brightness:", NULL),
		MENU_END
	};
	...
	for (;;) {
		...
		FindMenuItem(menu_array,7)->suffix = brightness_menu_array[brightness].item;
		SetItemChecked(menu_array, 1, Atari_GetAutoFrameSkip());
		...
		case 0:
			...
			break;
		case 1:
			Atari_AutoFrameSkip(!Atari_GetAutoFrameSkip());
			break;
		case 2:
			if (Atari_GetAutoFrameSkip() == 0)
				ui_driver->fMessage("No effect without frame skip");
...
#endif
		case 7:
			option2 = ui_driver->fSelect(NULL, SELECT_POPUP, brightness, brightness_menu_array, NULL);
			if (option2 >= 0) {
				Atari_DisplayBrightness(option2);
			}
			break;
		default:
			return;
...
#endif
		option = ui_driver->fSelect("Controller settings", 0, option, menu_array, NULL);
		switch (option) {
		case 0:
...
{
	static char filename[FILENAME_MAX] = "";
	if (ui_driver->fGetSaveFilename("png",filename, saved_files_dir, n_saved_files_dir)) {
#ifdef USE_CURSES
...
static void AboutEmulator(void)
{
	ui_driver->fInfoScreen("About the Emulator",
		...
		"(http://www.scrameta.net/a800.html\0"
		"for GP32X/Gizmondo)\0"
		"\0"
		"\0"
		"\0"
...
#define MENU_CONTROLLER 19
	static tMenuItem menu_array[] = {
		MENU_FILESEL_ACCEL(MENU_RUN, "Run Atari Program", ""),
		MENU_SUBMENU_ACCEL(MENU_DISK, "Disk Management", ""),
		MENU_SUBMENU_ACCEL(MENU_CARTRIDGE, "Cartridge Management", ""),
		MENU_FILESEL(MENU_CASSETTE, "Select Tape Image"),
		MENU_LABEL("---"),
		MENU_FILESEL_ACCEL(MENU_LOADSTATE, "Load State", ""),
		MENU_FILESEL_ACCEL(MENU_SAVESTATE, "Save State", ""),
		MENU_ACTION(MENU_STARTUPLIKETHIS, "Startup like this"),
		MENU_ACTION(MENU_DELETE, "Delete state files"),
		MENU_LABEL("---"),
		MENU_SUBMENU_ACCEL(MENU_SYSTEM, "Select System", ""),
		MENU_SUBMENU(MENU_CAPTURE, "Capture screen/sound"),
		MENU_LABEL("---"),
		MENU_SUBMENU_ACCEL(MENU_SOUND, "Sound Settings", ""),
		MENU_SUBMENU(MENU_DISPLAY, "Display Settings"),
		MENU_SUBMENU(MENU_SETTINGS, "Emulator Configuration"),
		MENU_LABEL("---"),
		MENU_ACTION_ACCEL(MENU_RESETW, "Reset (Warm Start)", ""),
		MENU_ACTION_ACCEL(MENU_RESETC, "Reboot (Cold Start)", ""),
		MENU_ACTION_ACCEL(MENU_EXIT, "Exit Emulator", ""),
		MENU_ACTION_ACCEL(MENU_ABOUT, "About the Emulator", ""),
		MENU_END
	};
	...
	while (!done) {
		...
		case MENU_STARTUPLIKETHIS:
		{
			char buffer[FILENAME_MAX];
			char * ptr = strcpy(buffer, saved_files_dir[0]);
#ifdef BACK_SLASH
			strcat(ptr, "\\autorun");
#else
			strcat(ptr, "/autorun");
#endif
			if (!SaveAtariState(buffer, "wb", TRUE))
				CantSave(buffer);
			break;
		}
		...
		case MENU_DELETE:
		{
			...
			static char filename[FILENAME_MAX] = "";
			if (ui_driver->fGetLoadFilename(filename, saved_files_dir, n_saved_files_dir)) 
			{
				static tMenuItem del_menu_array[] = 
				{
					MENU_ACTION(0,"Delete"),
					MENU_ACTION(1,"Cancel"),
					MENU_END
				};
				del_menu_array[0].item = filename;
				if (ui_driver->fSelect("Are you sure?", TRUE, 1, del_menu_array, NULL) == 0)
				{
					Util_unlink(filename);
				}
			}
		}
		break;
...
{
	static tMenuItem menu_array[] = {
		MENU_FILESEL_ACCEL(MENU_PCX, "Save Screenshot", ""),
		MENU_FILESEL_ACCEL(MENU_PCXI, "Save Interlaced Screenshot", ""),
		MENU_ACTION_ACCEL(MENU_SOUND_RECORDING, "Sound Recording Start/Stop", ""),
		MENU_END
	};
	...
	for (;;)
	{
		option = ui_driver->fSelect("Capture menu", 0, option, menu_array, NULL);
