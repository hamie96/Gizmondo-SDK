...
char atari_clockcommand_filename[FILENAME_MAX] = FILENAME_NOT_SET;
...
void addcwd(char * dest, char const * ptr)
{
		if (strlen(ptr)>0 && ptr[0]!='\\')
		{
			strcpy(dest, getcwd(0,0));
			strcat(dest,"\\");
		}
		else
			*dest = 0;
...
char const * removecwd(char const * ptr)
{
	char const * rawptr = ptr;
	char * cwd = getcwd(0,0);
	while (*cwd!=0 && ptr!=0 && *cwd==*ptr) ++ptr,++cwd;
	if (*cwd == 0)
		return ptr+1; // strip backslash
	else
		return rawptr;
}
...
{
	static char * const rom_filenames[] = {
		atari_osa_filename,
		atari_osb_filename,
		atari_xlxe_filename,
		atari_5200_filename,
		...
		atari_menu_filename,
		atari_uimenu_filename,
		atari_clockcommand_filename
	};
	static const char * const common_filenames[] = {
		...
		"ATARIBASIC.ROM", "ATARIBAS.ROM", "BASIC.ROM", "ATARI_BASIC.ROM",
		NULL,
		"default.men","DEFAULT.MEN","Default.men",NULL,
		"ui.men","UI.MEN","Ui.men",NULL,
		"cpuspeed.sh","cpu_speed.sh","clockspeed.sh","CPUSPEED.SH","CPU_SPEED.SH","CLOCKSPEED.SH", NULL
	};
...
int Atari800_ReadConfig(const char *alternate_config_filename)
{
	...
	/* else use the default config name under the HOME folder */
		...
		char cwdbuffer[256];
		if (getcwd(cwdbuffer, 256)!=0)
			Util_catpath(rtconfig_filename, cwdbuffer, DEFAULT_CFG_NAME);
		else
			Util_catpath(rtconfig_filename, "/mnt/sd/", DEFAULT_CFG_NAME);
	}
		...
		if (ptr != NULL) {
			*ptr++ = '\0';
			...
			if (strcmp(string, "OS/A_ROM") == 0)
				addcwd(atari_osa_filename, ptr);
			else if (strcmp(string, "OS/B_ROM") == 0)
				addcwd(atari_osb_filename, ptr);
			else if (strcmp(string, "XL/XE_ROM") == 0)
				addcwd(atari_xlxe_filename, ptr);
			else if (strcmp(string, "BASIC_ROM") == 0)
				addcwd(atari_basic_filename, ptr);
			else if (strcmp(string, "5200_ROM") == 0)
				addcwd(atari_5200_filename, ptr);
			else if (strcmp(string, "UI_MENU") == 0)
				addcwd(atari_uimenu_filename, ptr); 
			else if (strcmp(string, "MENU")==0)
				addcwd(atari_menu_filename, ptr);
			else if (strcmp(string, "CLOCK_COMMAND")==0)
				addcwd(atari_clockcommand_filename, ptr, sizeof(atari_clockcommand_filename));
#ifdef BASIC
				...
				addcwd(atari_h_dir[0], ptr);
			else if (strcmp(string, "H2_DIR") == 0)
				addcwd(atari_h_dir[1], ptr);
			else if (strcmp(string, "H3_DIR") == 0)
				addcwd(atari_h_dir[2], ptr);
			else if (strcmp(string, "H4_DIR") == 0)
				addcwd(atari_h_dir[3], ptr);
			...
			else if (strcmp(string, "DEFAULT_BRIGHTNESS") == 0) {
				defaultBrightness = atoi(ptr);
...
int Atari800_WriteConfig(void)
{
	...
	fprintf(fp, "%s\n", ATARI_TITLE);
	fprintf(fp, "OS/A_ROM=%s\n", removecwd(atari_osa_filename));
	fprintf(fp, "OS/B_ROM=%s\n", removecwd(atari_osb_filename));
	fprintf(fp, "XL/XE_ROM=%s\n", removecwd(atari_xlxe_filename));
	fprintf(fp, "BASIC_ROM=%s\n", removecwd(atari_basic_filename));
	fprintf(fp, "5200_ROM=%s\n", removecwd(atari_5200_filename));
	fprintf(fp, "UI_MENU=%s\n", removecwd(atari_uimenu_filename));
	fprintf(fp, "MENU=%s\n", removecwd(atari_menu_filename));
	fprintf(fp, "CLOCK_COMMAND=%s\n", atari_clockcommand_filename);
#ifndef BASIC
	for (i = 0; i < n_atari_files_dir; i++)
		fprintf(fp, "ATARI_FILES_DIR=%s\n", removecwd(atari_files_dir[i]));
	for (i = 0; i < n_saved_files_dir; i++)
		fprintf(fp, "SAVED_FILES_DIR=%s\n", removecwd(saved_files_dir[i]));
#endif
	for (i = 0; i < 4; i++)
		fprintf(fp, "H%c_DIR=%s\n", '1' + i, removecwd(atari_h_dir[i]));
	...
	fprintf(fp, "DEFAULT_BRIGHTNESS=%d\n", brightness); // store current as default
...
int Atari800_Initialise(int *argc, char *argv[])
{
	...
	if (!got_config)
	{
		...
		if (strcmp(cwdbuffer,"/mnt/sd") == 0)
			strcpy(cwdbuffer,"/mnt/sd/roms/a800/");

		Util_catpath(rompath, cwdbuffer, "os");
		Atari800_FindROMImages(rompath, TRUE);
		Util_catpath(rompath, cwdbuffer, "menu");
		Atari800_FindROMImages(rompath, TRUE);
		Util_catpath(rompath, cwdbuffer, "util");
		Atari800_FindROMImages(rompath, TRUE);
		Util_catpath(rompath, cwdbuffer, "rom");
		Atari800_FindROMImages(rompath, TRUE);

		Util_catpath(rompath, cwdbuffer, "disk");
		Util_strlcpy(atari_files_dir[n_atari_files_dir++], rompath, FILENAME_MAX);
		Util_catpath(rompath, cwdbuffer, "rom");
		Util_strlcpy(atari_files_dir[n_atari_files_dir++], rompath, FILENAME_MAX);
		Util_catpath(rompath, cwdbuffer, "exe");
		Util_strlcpy(atari_files_dir[n_atari_files_dir++], rompath, FILENAME_MAX);

		Util_catpath(rompath, cwdbuffer, "state");
		Util_strlcpy(saved_files_dir[n_saved_files_dir++], rompath, FILENAME_MAX);
	}
...
/* skip "ROM" on systems that are known to be case-insensitive */
	...
	if (Util_filenamenotset(atari_uimenu_filename))
		strcpy(atari_uimenu_filename, "ui.men");
	if (Util_filenamenotset(atari_menu_filename))
		strcpy(atari_menu_filename, "default.men");
	if (Util_filenamenotset(atari_clockcommand_filename))
		strcpy(atari_clockcommand_filename, "cpuspeed.sh");
