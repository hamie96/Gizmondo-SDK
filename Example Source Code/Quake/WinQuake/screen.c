...
float CalcFov (float fov_x, float width, float height)
{
        ...
        x = (float)(width/tan(fov_x/360*M_PI));
...
// redraw with no console and the loading plaque
	...
	scr_disabled_time = (float)realtime;
	scr_fullupdate = 0;
}
...
// draw a fresh screen
	...
	do
	{
		...
		Sys_SendKeyEvents ();
	} while (key_lastpress != 'Y' && key_lastpress != 'N' && key_lastpress != 'y' && key_lastpress != 'n' && key_lastpress != K_ESCAPE && key_lastpress != K_ENTER && key_lastpress != K_JOY1);

	scr_fullupdate = 0;
	SCR_UpdateScreen ();

	return key_lastpress == 'y'||key_lastpress == 'Y'||key_lastpress == K_ENTER||key_lastpress == K_JOY1;
}
