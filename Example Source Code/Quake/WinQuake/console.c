...
#define MAXGAMEDIRLEN	1000
	...
	if (con_debuglog)
	{
		...
		{
			TCHAR buf[MAXGAMEDIRLEN];
			sprintf (temp, "%s%s", com_gamedir, t2);
			//Dan: Unicode conversion and work-around lack of unlick function.
			MultiByteToWideChar(CP_ACP, 0, temp, -1, buf, sizeof(buf)/sizeof(TCHAR));
			DeleteFile(buf);
//			unlink (temp);
		}
	}
...
void Con_Print (char *txt)
{
	...
	// word wrap
		...
		// mark time for transparent overlay
			if (con_current >= 0)
				con_times[con_current % NUM_CON_TIMES] = (float)realtime;
		}
