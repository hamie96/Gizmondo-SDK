...
// set up gun position
	...
	for (i=0 ; i<3 ; i++)
	{
		view->origin[i] += (float)(forward[i]*bob*0.4);
...
// don't allow cheats in multiplayer
	...
	switch ((int)crosshair.value) {
		case 1:
			Draw_Character ((int)(scr_vrect.x + scr_vrect.width/2)-4,
				(int)(scr_vrect.y + scr_vrect.height/2 + cl_crossy.value), '+');
			break;
		case 2:	{ //Cross
			unsigned char uc = 254;
			unsigned char *center, *pos;
			int ystep=vid.rowbytes;
			center = vid.buffer + ((scr_vrect.height/2+4) * ystep + (scr_vrect.width/2));
			pos=center-4;
			*pos++=uc; *pos++=uc; *pos++=uc;

			pos=center+2;
			*pos++=uc; *pos++=uc; *pos++=uc;
			pos=center-4*ystep;
			*pos=uc; pos+=ystep; *pos=uc; pos+=ystep; *pos=uc; pos+=ystep;
			pos=center+(ystep*2);
			*pos=uc; pos+=ystep; *pos=uc; pos+=ystep; *pos=uc; pos+=ystep;
...
// don't allow cheats in multiplayer
	...
	switch (crosshair.value) {
		case 1:
			Draw_Character ((int)(scr_vrect.x + scr_vrect.width/2), 
				(int)(scr_vrect.y + scr_vrect.height/2 + cl_crossy.value), '+');
			break;
		case 2:	{ //Cross
			unsigned		uc = d_8to16table[0x00ffffff];
			unsigned short *center, *pos;
			int ystep=vid.rowbytes >> 1;
			center = (unsigned short *)vid.buffer + (vid.height/2) * step + (vid.width/2);
			pos=center-4;
			*pos++=uc; *pos++=uc; *pos++=uc;
			pos=center+1;
			*pos++=uc; *pos++=uc; *pos++=uc;
