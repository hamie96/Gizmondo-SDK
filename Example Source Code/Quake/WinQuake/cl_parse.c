...
// precache models
	memset (clFPM.model_precache, 0, sizeof(clFPM.model_precache));
	for (nummodels=1 ; ; nummodels++)
	{
		str = MSG_ReadString ();
		if (!str[0])
...
// precache sounds
	memset (clFPM.sound_precache, 0, sizeof(clFPM.sound_precache));
	for (numsounds=1 ; ; numsounds++)
	{
		str = MSG_ReadString ();
		if (!str[0])
...
void CL_NewTranslationFPM (int slot)
{
	...
	source = vid.colormap;
	memcpy (dest, vid.colormap, sizeof(clFPM.scores[slot].translations));
