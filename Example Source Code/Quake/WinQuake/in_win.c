...
//Dan: Most of this is not used for Pocket PC, and needs to be weeded out
...
qboolean IN_InitDInput (void)
{
	...
	if (!hInstDI)
	{
		hInstDI = LoadLibrary(_T("dinput.dll"));
	...
	{
		pDirectInputCreate = (void *)GetProcAddress(hInstDI,_T("DirectInputCreateA"));
...
void IN_Init (void)
{
	...
	uiWheelMessage = RegisterWindowMessage ( _T("MSWHEEL_ROLLMSG") );
	...
	for (i = 0; i < JOY_MAX_AXES; i++)
	{
		...
		if (joy_wwhack2.value != 0.0)
		{
			if (dwAxisMap[i] == AxisTurn)
			{
				...
				fTemp = (float)(300.0 * pow(abs((int)fAxisValue) / 800.0, 1.3));
				if (fTemp > 14000.0)
					fTemp = 14000.0;
		...
		case AxisTurn:
			...
			else
			{
				...
				if (fabs(fAxisValue) > joy_yawthreshold.value)
				{
					...
					{
						cl.viewangles[YAW] += (float)((fAxisValue * joy_yawsensitivity.value) * speed * 180.0);
					}
		...
		case AxisLook:
			if (in_mlook.state & 1)
			{
				if (fabs(fAxisValue) > joy_pitchthreshold.value)
				{
					...
					{
						cl.viewangles[PITCH] += (float)((fAxisValue * joy_pitchsensitivity.value) * speed * 180.0);
					}
					V_StopPitchDrift();
				}
				else
